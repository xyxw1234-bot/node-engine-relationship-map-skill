    ...
    for p in ROOT.rglob('*'):
        ...
        for pat in FORBIDDEN_PATTERNS:
            if re.search(pat, txt, flags=re.I):
                ...
                break
    if problems:
        fail('发现历史复杂路线或格式源码污染：'+json.dumps(problems[:80],ensure_ascii=False,indent=2))
...
def main():
    ...
    cleanup_cache()
    print(json.dumps({'passed':True,'version':'v2.4','mode':'relationship_view','outputs':outputs},ensure_ascii=False,indent=2))
