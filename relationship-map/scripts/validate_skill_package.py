...
for script_phrase in ['classify_intent', 'list_view', 'detail_view', 'return_to_list', 'propose_update', 'parse_open_request', 'metric_evidence', 'RelationshipStore', 'RelationshipCardAdapter', 'Hermes 原生', '不单独部署服务器', '$HERMES_HOME/data/relationship-map', 'extensions/', 'Adaptive Relationship Intelligence', 'infer_adaptive_dimensions']:
    if script_phrase not in all_text:
        print('缺少运行层能力:', script_phrase); sys.exit(1)
print('校验通过：v1.8.0 人脉地图发布级安全与运行层校验通过，未发现旧命名。')
