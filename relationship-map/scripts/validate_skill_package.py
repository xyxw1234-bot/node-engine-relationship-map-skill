...
required=[
 ...
 'plugins/relationship-map-feishu-card/plugin.yaml','plugins/relationship-map-feishu-card/__init__.py','scripts/test_relationship_map_feishu_plugin.py',
...
if not skill.startswith('---\n') or 'name: relationship-map' not in skill or 'version: 2.1' not in skill:
...
for p in root.rglob('*.md'):
    ...
    for m in re.finditer(r'\[查看详情\]|新增联系人｜搜索', txt):
        window=txt[max(0,m.start()-30):m.end()+30]
        if not any(k in window for k in ['不得','禁止','不是','没有','失败','伪按钮','不能']):
            print('发现正向伪按钮示例', p, window); sys.exit(1)
print('校验通过：v2.1 人脉地图发布级安全、真实飞书卡片与运行层校验通过。')
